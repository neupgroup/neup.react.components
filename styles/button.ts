...
export type variant = 'solid' | 'tinted' | 'outlined' | 'plain' | 'text'
...
export function buttonVariants({ variant = 'tinted', convey = 'none', size = 'default', alignment = 'center', className }: ButtonStyleProps = {}) { return cn('inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0', variants[variant], convey !== 'none' && variant === 'solid' ? conveys[convey] : undefined, sizes[size], alignments[alignment], className) }
